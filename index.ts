...
const BASE_URL = "https://db.indiecloud.co";
type Action =
	| "put"
	| "get"
	| "merge"
...
interface IQuipuBase<T> {
	...
	fetch(action: Action, data?: T, key?: string, limit?: number, offset?: number): Promise<Status | T | T[]>;
...
export class QuipuBase<T> implements IQuipuBase<T> {
	...
	async fetch(action: Action,
		...
		const url = this.buildUrl(action, key, limit, offset);
